...
if model.train_embeddings:
    logit = model.rho(alpha.view(alpha.size(0)*alpha.size(1), model.rho_size))
else:
    tmp = alpha.view(alpha.size(0)*alpha.size(1), model.rho_size)
    logit = torch.mm(tmp, model.rho.permute(1, 0)) 
logit = logit.view(alpha.size(0), alpha.size(1), -1)
beta = F.softmax(logit, dim=-1)
