...
class DMETM(nn.Module):
    ...
    def get_alpha(self): ## mean field for multivariate Gaussian one dimension per source
        ...
        # at time 0, pass mean and variance for all topics, all embeddings, all countries
        alphas[0] = self.reparameterize(self.mu_q_alpha[:, 0, :, :], self.logsigma_q_alpha[:, 0, :, :])

        p_mu_0 = torch.zeros(self.num_sources, self.num_topics, self.rho_size, self.num_sources).to(device)
        
        logsigma_p_0 = torch.zeros(self.num_sources, self.num_topics, self.rho_size, self.num_sources).to(device)
