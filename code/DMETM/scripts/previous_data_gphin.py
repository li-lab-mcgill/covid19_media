...
def read_data(data_file):
    ...
    land_screenings = df_test['LAND SCREENING'].values
    border_closings = df_test['BORDER CLOSING'].values
    airport_screening_entries = df_test['AIRPORT SCREENING: ENTRY'].values
    airport_screening_exits = df_test['AIRPORT SCREENING: EXIT'].values
    testing_cases=df_test['TESTING & CASE DETECTION  '].values
    quarantine_monitors=df_test['QUARANTINE / MONITORING'].values
    travel_advisory=df_test['TRAVEL ADVISORY'].values
    travel_bans_cancellation=df_test['TRAVEL BAN / CANCELLATION'].values
    trade_bans=df_test['TRADE BANS'].values

    education_campains=df_test['EDUCATION CAMPAIGN'].values
    testing_cases=df_test['TESTING & CASE DETECTION  '].values
    mass_gatherings=df_test['MASS GATHERING CANCELLATION'].values
    limit_gatherings=df_test['RESTRICTING OR LIMITING GATHERINGS'].values
    close_public_spaces=df_test['CLOSING PUBLIC PLACES'].values
    lock_or_curfews=df_test['LOCKDOWN OR CURFEW'].values

    ease_restrictions=df_test['EASIND RESTRICTIONS'].values
    vaccines=df_test['VACCINE/MCM DEPLOYED'].values
    ppes=df_test['PPE'].values
...
def get_features(docs, stops, timestamps, sources, labels, min_df=min_df, max_df=max_df):
    ...
    return vocab, word2id, id2word, time2id, id2time, time_list, cvz, source_map, labels_map
...
def split_data(cvz, docs, timestamps, word2id, countries, source_map, labels, labels_map):
...
def save_data(save_dir, timestamps_tr, timestamps_ts, timestamps_va ,time_list, bow_tr, bow_ts, bow_ts_h1, bow_ts_h2, bow_va, vocab, n_docs_tr, n_docs_ts, n_docs_va, c_tr, c_ts, c_ts_h1, c_ts_h2, c_va, source_map,l_tr, l_ts, l_ts_h1, l_ts_h2, l_va, labels_map, min_df=min_df):
    ...
    # save the labels to id mapping
    print(labels_map.values())
    pickle.dump(labels_map, open(path_save + "labels_map.pkl","wb"))
...
if __name__ == '__main__':
    ...
    all_docs, all_times, all_countries, twoDarray = read_data(args.data_file_path)
    ...
    vocab, word2id, id2word, time2id, id2time, time_list, cvz, source_map, labels_map = get_features(all_docs, stopwords, all_times, all_countries, twoDarray)
    print(source_map)

    # split data into train, test and validation and corresponding countries in BOW format
    bow_tr, n_docs_tr, bow_ts, n_docs_ts, bow_ts_h1, n_docs_ts_h1, bow_ts_h2, n_docs_ts_h2, bow_va, n_docs_va, timestamps_tr, timestamps_ts, time_ts_h1, time_ts_h2, timestamps_va, c_tr, c_ts, c_ts_h1, c_ts_h2, c_va, l_tr, l_ts, l_ts_h1, l_ts_h2, l_va = split_data(cvz, all_docs, all_times, word2id, all_countries, source_map, twoDarray, labels_map)

    save_data(args.save_dir, timestamps_tr, timestamps_ts, timestamps_va ,time_list, bow_tr, bow_ts, bow_ts_h1, bow_ts_h2, bow_va, vocab, n_docs_tr, n_docs_ts, n_docs_va, c_tr, c_ts, c_ts_h1, c_ts_h2, c_va, source_map, l_tr, l_ts, l_ts_h1, l_ts_h2, l_va, labels_map)
    print('Data ready !!')
    print('*************')