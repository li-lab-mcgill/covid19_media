...
train_countries = train['countries']
args.num_docs_train = len(train_tokens)

# 2. dev set
valid_tokens = valid['tokens']
valid_counts = valid['counts']
valid_countries = valid['countries']
...
test_1_countries = test['countries_1']
args.num_docs_test_1 = len(test_1_tokens)
test_2_tokens = test['tokens_2']
test_2_counts = test['counts_2']
test_2_countries = test['countries_2']
...
def visualize(m, all_countries, show_emb=True):
    ...
    with torch.no_grad():
        ...
        topics_words = []
        gammas = m.get_beta(all_countries[0])
...
def evaluate(m, all_countries, source, tc=False, td=False):
    ...
    with torch.no_grad():
        ...
        ## get \beta here
        beta = m.get_beta(all_countries[0])
        ...
        for idx, ind in enumerate(indices_1):
            ...
            data_batch_1, data_countries_1 = data.get_batch(test_1_tokens, test_1_counts, test_1_countries, ind, args.vocab_size, device)
            ...
            data_batch_2, data_countries_2 = data.get_batch(test_2_tokens, test_2_counts, test_2_countries, ind, args.vocab_size, device)
...
if args.mode == 'train':
    ...
    visualize(model, all_countries)
    print('\n')
    for epoch in range(1, args.epochs):
        train(epoch)
        val_ppl = evaluate(model, all_countries, 'val')
    ...
    with torch.no_grad():
        ...
        test_ppl = evaluate(model, all_countries, 'test', tc=args.tc, td=args.td)
