...
class ETM(nn.Module):
    ...
    def forward(self, bows, normalized_bows, countries, theta=None, aggregate=True):
        ## get \theta
        if theta is None:
        ...
        ## get \beta   
        #beta = self.get_beta(country)
        total_loss = 0.

        for i, country in enumerate(countries):
            beta = self.get_beta(country)
            preds = self.decode(theta[i].reshape(1,theta.shape[1]), beta)
            recon_loss = -(preds * bows[i]).sum(1)
