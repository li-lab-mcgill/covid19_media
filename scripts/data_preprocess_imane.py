...
def get_args():
	...
	parser.add_argument("--data_file_path", type=str, default='data/who_updated.csv')
...
def read_data(data_file, who_flag=False, full_data=False, coronanet_flag=False):
	...
	#gphin_data['SUMMARY'] = " ".join(w for w in nltk.wordpunct_tokenize(gphin_data['SUMMARY']) if w.lower() in words or not w.isalpha())
	gphin_data = gphin_data[gphin_data['SUMMARY'].notna()]
	data_ids = gphin_data.index.values
	#print(gphin_data.columns)
	if not full_data:
		gphin_data.dropna(subset=['SOURCE', 'DATE ADDED'], inplace=True)
		#gphin_data = gphin_data[gphin_data[['COUNTRY /ORGANIZATION','DATE ADDED']].notna()]
		...
		if who_flag:
			...
			label_map = {'Anti-Disinformation Measures':0,'Closure and Regulation of Schools':1,'Curfew':2,'Declaration of Emergency':3, 'External Border Restrictions':4,'Health Monitoring':5, 'Health Resources':6,'Health Testing':7, 'Hygiene':8, 'Internal Border Restrictions':9,'Lockdown':10, 'New Task Force, Bureau or Administrative Configuration':11, 'Other Policy Not Listed Above':12, 'Public Awareness Measures':13, 'Quarantine':14, 'Quarantine/Lockdown':15, 'Restriction and Regularion of Businesses':16, 'Restriction and Regularion of Government Services':17, 'Restrictions of Mass Gatherings':18, 'Social Distancing':19}
		else:
